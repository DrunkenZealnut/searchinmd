...
- **재세그먼트 (2026-09-06) — "절단 16쪽" 해석 폐기, 페이지 수치 전면 재계산.** 외부감사(2026-09-04) C1: 워크북의 페이지 라벨은 2026-04 검색 당시 목차 유도 마커 때문에 여러 쪽을 묶은 블록이었다(32,767자 라벨 16개는 잘린 쪽이 아니라 그 증상). `resegment.py` 가 원본 PDF 86권 중 마크다운이 있는 84권(8,861쪽)의 쪽 텍스트에 마크다운 줄을 정렬해(마크다운 없는 2권 51쪽은 구 라벨 유지) 7,769행을 실제 쪽에 재배치(이동 4,316행, 미매칭 94행)하고 `regrade.py` 기준선으로 쪽 등급을 다시 냈다: 검출 1,847→**2,173쪽**, 등급3 108(5.8%)→**147쪽(6.8%)**, 등급3 0쪽 교재 49→57권, 사고사례 판정 8→13쪽(실제 산업재해 1건은 그대로). 정렬 정확도(마커 보유 23권 자기 검증) DP 후보 줄 기준 83.6%(±1쪽 94.9%), 전체 본문 줄 기준 77.6%(90.3%), 1:1 쪽 등급 일치 98.7%. 보고서(`data/…(0905)_…_수정0906.hwpx`)에는 반영했고 **대시보드·README·`summary.json` 은 라벨 기준 그대로** — 교체 여부는 연구 책임자 판단(`resegment-results.analysis.md` §6). 재코딩 수치는 보고서에 싣지 않기로 결정(2026-09-06).
- **재세그먼트 열린 판단 2건 (2026-09-06, 출하 전 리뷰).** ① 짧은 정형구 카운터 — `match_rows` 의 10자 미만 정형구 행(예: '안전 · 유의 사항')은 (시트, 전체 키=제목 맥락 포함)으로 묶여, 제목 맥락이 다른 같은 정형구 행은 각각 첫 등장 줄부터 센다. (시트, 정형구)로 묶어 문서 순서대로 서로 다른 등장에 놓는 대안을 실측하면 2,173→**2,178쪽**(등급1 1,502→1,507, 등급2·3 불변, 영향 3권, 이동 행 4,316→4,293) — 등급3 147쪽(6.8%)은 어느 쪽이든 같다. 현행 동작은 R16 픽스처로 고정돼 있고, 바꾸려면 `EXPECTED`·`reseg_summary.json`·결과 문서 §3 을 함께 갱신해야 한다(`--force`). 연구 책임자 판단.
  ② **마커 결손 쪽의 하이브리드 배정.** 마커 교재 23권은 마커를 그대로 쓰는데 마커가 빠진 쪽(밀도 0.8 이상이면 통과)의 줄이 앞 쪽에 붙는다. CSV `md자수`/`pdf자수`로 마크다운 본문이 PDF 쪽 본문의 1.7배를 넘는 쪽이 59쪽(마커 교재 48, 등급 1/2/3 = 28/19/12, 최악 9,140자 대 219자). 리뷰 추정으로 DP 로 쪼개 재채점하면 등급3 −2쪽·등급2 −4쪽. 마커 사이가 2쪽 이상 비면 DP 를 쓰는 하이브리드는 `EXPECTED`·산출물·보고서 표 11·12 갱신이 따른다 — 연구 책임자 판단 (`resegment-results.analysis.md` §4·§6).
...
- **"5.8%는 상한"이 아니다 — 이제 실측 근거가 있다.** 결함들이 서로 상쇄한다: D2 정규화 **-36쪽**, 셀 한도 절단 **폐기**(잘린 쪽이 아니라 여러 쪽을 묶은 라벨이었다 — 재세그먼트 후 등급3 108→147쪽, `docs/03-analysis/resegment-results.analysis.md`), D1 단어 경계 **0**(C-2 수정 후), 그리고 **어휘 누락 +21쪽**. 어느 하나만 고치고 발표하면 방향이 편향된다. 알려진 결함은 **절단과 어휘를 빼면** 과대계상 축(정밀도)이고, 재현율은 2026-09-04 에 처음 측정됐다(Claude 21% / GPT 13%, 두 계열 독립). **절단 축은 폐기됐다**(2026-09-06) — 잘린 쪽이 아니라 여러 쪽을 묶은 라벨이었고, 실제 쪽으로 풀면 등급3이 108→147쪽으로 오히려 는다(`resegment-results.analysis.md` §3.3). 4번을 수행하면 비율이 **올라갈** 수 있다. 발행 가능한 진술은 "5.8%는 결함이 확인된 현행 규칙의 출력값이며, 두 계열의 AI 코더 기준 재현율이 13~21%(진짜 등급3 22~37%)라 참값의 상한으로 해석할 수 없다. 사람 코딩은 아직 없다" 까지다.

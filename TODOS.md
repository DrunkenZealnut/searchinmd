...
- **Context**:
  1. ~~**포함 표현 75개 도메인 점검 (감사 M1).**~~ **닫힘 (2026-09-14, `semantic-expression-review`)** — 결함 2건 반영(v1fix), 22개 표현 × 30건 정밀도 점검(코더 2계열 κ 0.965), 사전 v2 채택(보류 방진화·케미컬, 조건부 방진복·장갑·X선·PSM, NCS 12,506 → 11,517). 남은 것: (a) `가연성` 28/30 경계 — 60건 2차 패스, (b) 정확 키워드의 문맥 정밀도 대조 표본(`화학물질`·`누출`·`작업환경`·`보호구` — 동의어 `화학약품`·`화학 물질`·`누설`·`작업 환경` 의 낮은 정밀도가 확장 오류인지 키워드 자체의 문맥인지 가르는 근거; `expression-review.analysis.md` §5·§7), (c) 동반어 목록 갱신 근거 축적(`scores.json.meta.companions`) — 특히 `노출`(노광)·`보호`(보호막)·`화상`(image) 의 동음이의: 교과서 `PSM` 1건·`X선` 2건이 공정 문맥인데 동반어로 살아남았다(ship 레드팀 2026-09-14), (e) 표본 시트의 «…» 표시가 줄의 첫 출현에 붙어 같은 줄 같은 표현 2회인 12쌍(24건)은 두 번째 출현을 따로 보여주지 못한다 — `ReviewRecord` 에 문자 위치를 더하고 24건만 재판정(키 digest 변경 → 라벨 파일 재생성 절차 필요; Codex 적대적 리뷰 2026-09-14), ~~(d) NCS 분야별 쪽수는 마커 최댓값 합(8,914)이라 PDF 쪽수보다 하한~~ **닫힘 (2026-09-15, `occurrence-real-pages` D4)** — 분야별 쪽수는 PDF 쪽수(84권 8,861 + 표식이 실제 쪽인 2권 239 = 9,100).
  2. ~~**페이지 단위 이질성 (감사 M2).**~~ **닫힘 (2026-09-15, `occurrence-real-pages`, 연구책임자 D1~D6 전부 A)** — `resegment.py` 의 줄→쪽 대응(`data/markdown/ncs_paged/`, 84권)을 `semantic_keyword_recount.py --page-maps` 로 넣어 NCS 출현 전부를 실제 PDF 쪽에 놓고 그 쪽 본문에 regrade 기준선 하나로 판정(`real-page`; 워크북 라벨 상속 없음). 새 2권은 표식이 실제 쪽(`REAL_PAGE_MARKER_BOOKS`). 정본: 등급 4,378/4,614/2,525 → 3,788/5,227/2,502(등급3 21.9 → 21.7%), 이동 4,016건, 이전 기준과 공유 쪽 2,035개 등급 100% 일치(`meta.run.reseg_agreement`), 영향표 `docs/03-analysis/data/occurrence_real_pages_impact.json`. 남은 것: 정렬 오차(±1쪽 약 5%)는 이전 기준과 같은 수준 — ④ 마커 보정 결정과 함께.
  3. ~~**비단조 마커 레거시 1권 재유도 여부.**~~ **닫힘 (2026-09-15, `occurrence-real-pages` D5)** — `LM1903060113` 의 출현도 정렬 대응(`pages.json`)으로 실제 쪽에 놓이므로 표식 순서는 더 이상 쪽 배정에 쓰이지 않는다. `meta.run.marker_nonmonotone` 기록은 그대로(매칭 블록 경계는 표식이라 정보로 남긴다).
  4. **`public_path()` 사본 통합 (ship 리뷰 D3) + 2026-09-14 ship 자문 5건.** `semantic_keyword_recount.public_path` 는 `resegment.public_path` 의 15줄 사본이고 `hwpx_results_refresh.public()`·`fmt`/`pct`(3곳)도 같은 부류. `page_utils` 로 옮기고 전부 거기서 import 할 것 — `resegment.py`·R16 테스트를 건드리므로 별도 정리 커밋. 같은 커밋에 `Facts.all_numbers` 래퍼 삭제, `hwpx_results_refresh` 표 명세 5-튜플→3-튜플, `expression_review.kappa` 를 `score_coding.kappa` 로(NaN→None 만 감싸기), `window_text(document, line, matched_text)` 시그니처 단순화 (연구책임자 2026-09-14: 이번 PR 에서는 보류). 2026-09-15 ship 자문 1건 추가(연구책임자: 보류): `run_census` 가 EXPECTED 가드용으로 계산한 metrics 를 버리고 `main()` 이 `AnalysisResult.run` dict 에서 `PageMapsInfo` 를 되살려 manifest 해시 + metrics 를 다시 계산한다(약 1 s) — `run_census` 가 metrics 를 함께 돌려주면 `AnalysisResult.run` 의 untyped dict 왕복이 사라진다.
  ...
  6. **조건부 규칙의 겹친 짧은 매칭이 "동반어 없음" 제외로 기록된다 (ship 적대적 리뷰 F11, 2026-09-14).** 같은 키워드의 더 긴 포함 규칙이 같은 구간을 덮어도(예: `고무장갑` 포함 + `장갑` 동반어 없음 제외) 짧은 조건부 규칙의 제외 레코드가 남는다 — v1fix 는 겹친 짧은 매칭을 조용히 버렸으므로 v2 의 제외 장부(보고서·xlsx 상세, 후보 감사, `impact.excluded_by_v2`)가 과계상된다. 발표되는 `excluded` 는 정확 티어만이라 대시보드 수치는 무관. 고치려면 포함 레코드가 덮는 구간의 제외 후보를 버려야 하는데 `detail_sha256` 이 바뀌어 정본 재실행·`EXPECTED` 재고정이 필요하다 — 정본 워크북(`20260402_재판정_20260414.xlsx`)이 `data/` 밖으로 옮겨져 이번 PR 에서는 보류. 관련: 사전 v2 의 동의어 4개(`화학약품`·`누설`·`화학 물질`·`작업 환경`)와 조건부 2개(`방진복` 5/10·`PSM` 1/2)는 측정 정밀도가 0.8 하한 아래인데 채택됐다(`expression-review.analysis.md` §5 가 명시; F15) — 보고서의 "사전 v2" 문구가 하한 충족을 뜻하지 않는다는 점은 그 문서에만 적혀 있다.
- **Depends on**: 1(a)(b)(c)(e) 는 연구 절차(코더 재실행), 4·5 는 결정 불필요, 6 은 정본 워크북이 2026-09-15 `data/` 로 돌아왔으므로 재실행 가능(`detail_sha256` 재고정 수반). 원본 PDF 86권도 2026-09-15 부터 `data/markdown/ncs/pdf/<분야>/`(비추적)에 있어 `NCS_PDF_ROOT=data/markdown/ncs/pdf` 로 `resegment.py` 를 로컬에서 돌릴 수 있다 — ④ 마커 보정과 새 2권 대응 생성(occurrence-real-pages D2-B)은 그 뒤 결정.
...
- **Status**: `regrade.py` reproduces the rule and fixes the two real defects; per-defect impact is in `docs/03-analysis/data/regrade_impact.json`. **The dashboards still publish the uncorrected (현행) rule's numbers** — on re-segmented pages since 2026-09-06, but with none of the D1/D2/D4/V corrections applied; applying one is a research-facing decision, not a code one.

  C-2(차감 산술) 수정 후 수치다. F1 열은 무효 라벨에서 나온 **폐기 예정 탐색값**이며 발행 근거가 아니다. 어느 행도 채택되지 않았다.

  | | 등급1 | 등급2 | 등급3 | 등급3 비율 | F1 (폐기 예정) |
  |---|---:|---:|---:|---:|---:|
  | 라벨 기준 원본 (2026-09-06 이전 발표; 변형 표 전체가 라벨 기준) | 1,270 | 469 | 108 | 5.8% | 0.803 / 0.810 |
...
- **재세그먼트 (2026-09-06) — "절단 16쪽" 해석 폐기, 페이지 수치 전면 재계산.** 외부감사(2026-09-04) C1: 워크북의 페이지 라벨은 2026-04 검색 당시 목차 유도 마커 때문에 여러 쪽을 묶은 블록이었다(32,767자 라벨 16개는 잘린 쪽이 아니라 그 증상). `resegment.py` 가 원본 PDF 86권 중 마크다운이 있는 84권(8,861쪽)의 쪽 텍스트에 마크다운 줄을 정렬해(마크다운 없는 2권 51쪽은 구 라벨 유지) 7,769행을 실제 쪽에 재배치(이동 4,270행, 미매칭 94행)하고 `regrade.py` 기준선으로 쪽 등급을 다시 냈다: 검출 1,847→**2,189쪽**, 등급3 108(5.8%)→**145쪽(6.6%)**(Act-3 하이브리드 배정 후), 등급3 0쪽 교재 49→57권, 사고사례 판정 8→13쪽(실제 산업재해 1건은 그대로). 정렬 정확도(마커 보유 23권 자기 검증) DP 후보 줄 기준 83.6%(±1쪽 94.9%), 전체 본문 줄 기준 77.6%(90.3%), 1:1 쪽 등급 일치 98.7%. 보고서(`data/…(0905)_…_수정0906.hwpx`)에는 반영했고, 2026-09-06 결정으로 대시보드·README 도 이 수치를 발표한다(이전 문구: **대시보드·README·`summary.json` 은 라벨 기준 그대로** — 교체 여부는 연구 책임자 판단(`resegment-results.analysis.md` §6)). 재코딩 수치는 보고서에 싣지 않기로 결정(2026-09-06).
- **재세그먼트 판단 2건 — 2026-09-06 연구 책임자 결정(번호는 `docs/archive/2026-09/resegment-publish/resegment-publish.plan.md` 기준): ③ 정형구 카운터 현행 유지, ② 하이브리드 채택(Act-3); ① 수치 교체는 위 항목.** ③ 짧은 정형구 카운터 — `match_rows` 의 10자 미만 정형구 행(예: '안전 · 유의 사항')은 (시트, 전체 키=제목 맥락 포함)으로 묶여, 제목 맥락이 다른 같은 정형구 행은 각각 첫 등장 줄부터 센다. (시트, 정형구)로 묶어 문서 순서대로 서로 다른 등장에 놓는 대안을 실측하면(Act-2 기준 실측, 하이브리드 이전) 2,173→**2,178쪽**(등급1 1,502→1,507, 등급2·3 불변, 영향 3권, 이동 행 4,316→4,293) — 등급3 147쪽(6.8%)은 어느 쪽이든 같다. 현행 동작은 R16 픽스처로 고정돼 있고, 바꾸려면 `EXPECTED`·`reseg_summary.json`·결과 문서 §3 을 함께 갱신해야 한다(`--force`). 연구 책임자 판단.
  ④ **마커 교재의 마커 ±1쪽 보정 여부 — 실측 완료, 결정 대기 (marker-offset, 2026-09-07).** 23권 2,035 마커 쪽의 본문을 PDF P±3 과 3-gram 대조(`marker_offsets`, DP 와 독립): 같은 쪽 1,772(87.1%), +1 63, −1 30, 모호 59, ±2·3 99, 못 잼 12 — 뚜렷한 ±1 93쪽 중 33 이 『반도체 장비 안전관리』(연속 구간 55~61 −1, 112~119·142~147 +1), −1 의 20 은 22권의 꼬리 마커(빈 뒷면). 진단은 산출물에 실렸고(`reseg_summary.json` `marker_offset`, CSV `마커오프셋`, `EXPECTED` 17키) 발표 수치는 불변. 보정 변형 `--marker-correct 0.05/0.10/0.20` 은 2,189→2,181쪽, 등급3 145→147(0.20 은 146), 사고사례 112→113(수기 표 일치 4/7→5/7), 자기 검증 nogap 88.6→90.5% — 임계에 둔감. 채택하면 `EXPECTED`·대시보드 사고사례 표·README·보고서가 바뀐다(`resegment-results.analysis.md` §3.7·§6 6; 권고 마진 0.10). 연구 책임자 판단.
  ② **마커 결손 쪽의 하이브리드 배정 — 채택(Act-3).** 마커 교재에서 마커 사이가 2쪽 이상 비면 그 사이 줄을 DP 배정으로 쪼개는 `hybrid_pages` 를 넣었다(구간 첫 줄의 DP 드리프트를 빼는 앵커 보정 포함): 본문 1,575줄 이동, 검출 2,173→2,189쪽, 등급3 147→145쪽, md 본문이 PDF 쪽의 1.7배를 넘는 쪽 59→47. `EXPECTED`·산출물·대시보드·보고서 표 10~12 를 함께 갱신했다(`resegment-results.analysis.md` §3.6).
...
- **감사 권고 중 거부한 것 1건.** 대시보드 경고 배너 게시 — 연구 책임자 판단으로 **달지 않기로 결정**. 대시보드는 경고 배너 없이 공개 상태를 유지한다(2026-09-06 부터 수치는 실제 쪽 기준 145쪽(6.6%); 라벨 기준 "108쪽(5.8%)" 은 "이전 발표" 로 한 번만 언급).
- **Depends on**: A decision on whether to republish, and at which number. 열려 있는 것 넷:
  ...
  4. **어휘 누락 탐색 완료 (2026-09-04)** — `docs/03-analysis/vocab-search.analysis.md`. **등급3을 5.8% → 7.0%(108→129쪽) 올린다.** 거의 전부 조치어 효과다(조치어만 +20, 안전어만 +1) — 등급3이 조치어 ≥5를 요구하므로 예측대로였다. 채택 후보 21종(조치 12 / 안전 9), 보류 6종(`접지`·`정전기`·`감지기`·`노출`·`취급`·`인체` — 동음이의), 기각 2종(`LOTO`·`산소결핍`은 본문에 아예 없다). 사전 역설계 자체는 정확했다 — 원본 채점기가 `등급사유`에 이름 붙인 용어는 **0종 누락**이고, 빠진 것은 원본 채점기의 어휘 자체다. **채택 여부는 연구 책임자 판단**이며, 다만 재코딩 표본은 어휘를 확정하는 대신 두 어휘 정의의 **합집합**으로 뽑아 채택을 라벨에 위임했다(`docs/archive/2026-09/recoding/recoding.design.md` §3; 확장 시 분쟁군 36→39, 교집합 31쪽뿐이라 합집합이 필요했다). → **2026-09-04 두 어휘 정의의 합집합으로 표본을 뽑아 어휘 확정 없이 진행했다**(분쟁군 44 = 36 ∪ 39). 라벨 결과 V 어휘 확장은 Claude·GPT 두 계열 모두 F1 1위(0.39 / 0.27)이나 채택은 여전히 열려 있다 — `recoding-results.analysis.md` §5.

- **"5.8%는 상한"이 아니다 — 이제 실측 근거가 있다.** 결함들이 서로 상쇄한다: D2 정규화 **-36쪽**, 셀 한도 절단 **폐기**(잘린 쪽이 아니라 여러 쪽을 묶은 라벨이었다 — 재세그먼트 후 등급3 108→145쪽, `docs/03-analysis/resegment-results.analysis.md`), D1 단어 경계 **0**(C-2 수정 후), 그리고 **어휘 누락 +21쪽**. 어느 하나만 고치고 발표하면 방향이 편향된다. 알려진 결함은 **절단과 어휘를 빼면** 과대계상 축(정밀도)이고, 재현율은 2026-09-04 에 처음 측정됐다(Claude 21% / GPT 13%, 두 계열 독립). **절단 축은 폐기됐다**(2026-09-06) — 잘린 쪽이 아니라 여러 쪽을 묶은 라벨이었고, 실제 쪽으로 풀면 등급3이 108→145쪽으로 오히려 는다(`resegment-results.analysis.md` §3.3·§3.6). 4번을 수행하면 비율이 **올라갈** 수 있다. 발행 가능한 진술은 "6.6%(실제 쪽 기준; 라벨 기준 5.8%)는 결함이 확인된 현행 규칙의 출력값이며, 두 계열의 AI 코더 기준 재현율이 13~21%(진짜 등급3 22~37%)라 참값의 상한으로 해석할 수 없다. 사람 코딩은 아직 없다" 까지다.

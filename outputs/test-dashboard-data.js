...
const makeContext = () => ({ document, console: { log() {}, warn() {}, error(...a) { throw Error(a.join(' ')); } }, window: null, globalThis: null, JSON, Math, Object, Array, String, Number, Date, RegExp, Intl, parseInt, parseFloat, isNaN, setTimeout, clearTimeout, getComputedStyle() { return { getPropertyValue() { return '#000'; } }; }, Chart });   // vm 샌드박스 — 렌더러가 쓰는 전역은 여기 한 곳에만
...
check('S1t 브리지 절이 실제 쪽 기준·닿은 쪽 수(출현 vs 이전 기준)·공유 쪽 등급 일치 N/N (100%)·차이의 이유(검출 사전 / 집계 단위·가중)를 제 문단으로 말한다 (occurrence-real-pages)', D.meta.page_basis.NCS === 'real' && /<p class="bridge-note"[^>]*>2026-09-15 부터 NCS 출현은/.test(ncs) && ncs.includes('같은 실제 PDF 쪽 기준') && ncs.includes('출현이 놓인 실제 쪽 ' + fmt(N.detected_pages) + '개, 이전 기준의 검출 쪽 ' + fmt(R.pages) + '개') && ncs.includes('둘이 공유하는 쪽의 등급 일치 ' + fmt(S.meta.run.reseg_agreement.agree) + '/' + fmt(S.meta.run.reseg_agreement.pages) + ' (' + pct(S.meta.run.reseg_agreement.agree, S.meta.run.reseg_agreement.pages) + '%)') && ncs.includes('집계 단위(출현건수 vs 쪽)와 그에 따른 가중') && !ncs.includes('같은 쪽 모집단') && !ncs.includes('분모(출현건수 vs 쪽)뿐'));
check('S1v 데이터 주의 등급 출처 — NCS 는 실제 쪽 판정 N건만(0건인 승계·신규는 적지 않는다), 교과서는 승계·신규만, 마커 없는 출현 0건은 둘 다 적는다', ncs.includes('실제 쪽 판정 ' + fmt(N.grade_sources['real-page']) + '건') && !ncs.includes('기존 판정 승계 0건') && !ncs.includes('신규 판정 0건') && ncs.includes('마커 없는 출현 문맥 판정 0건') && !school.includes('실제 쪽 판정') && school.includes('기존 판정 승계 ' + fmt(T.grade_sources.existing) + '건 · 신규 판정 ' + fmt(T.grade_sources['new']) + '건 · 마커 없는 출현 문맥 판정 0건'));
...
const noMap = S.meta.run.real_page_marker_books || [];   // 대응 없이 표식이 실제 쪽인 교재 [{code, pages}] — manifest 가 말한다 (하드코딩 239 아님)
check('S3n NCS 그룹 pages 합 == reseg per_book.pdf_pages 합(84권) + manifest 의 대응 없는 교재 표식 최댓값 합 (D4)', Array.isArray(noMap) && noMap.length === 2 && noMap.every((b) => /^LM\d{10}$/.test(b.code) && b.pages > 0) && N.groups.reduce((a, g) => a + g.pages, 0) === Object.values(R.per_book).reduce((a, b) => a + (b.pdf_pages || 0), 0) + noMap.reduce((a, b) => a + b.pages, 0) && S.meta.page_basis.NCS === 'real' && S.meta.page_basis['교과서'] === 'marker', JSON.stringify(noMap));
const I = JSON.parse(read('docs/03-analysis/data/occurrence_real_pages_impact.json'));
check('S3o 영향표(occurrence_real_pages_impact.json)는 정본과 같은 대응 지문·총계·실제 쪽 등급·닿은 실제 쪽 수', I.meta.inputs.page_maps.sha256 === S.meta.run.page_maps.sha256 && I.totals.NCS === N.total && I.totals['교과서'] === T.total && ['1', '2', '3'].every((g) => I.grades.real.NCS[g] === N.grades[g] && I.grades.real['교과서'][g] === T.grades[g]) && I.pages.real_pages === N.detected_pages);
...
check('S5h index.html 정적 히어로의 공유 쪽 등급 일치 == summary reseg_agreement (브리지와 같은 N/N (100%) 형식)', vNcs.includes('공유 쪽 등급 일치 ' + fmt(S.meta.run.reseg_agreement.agree) + '/' + fmt(S.meta.run.reseg_agreement.pages) + ' (' + pct(S.meta.run.reseg_agreement.agree, S.meta.run.reseg_agreement.pages) + '%)'));
...
check('S6f 결과 페이지 meta 줄이 페이지 열의 뜻을 말한다 (NCS 실제 PDF 쪽 · 교과서 표식) — page_basis real 일 때', S.meta.page_basis.NCS === 'real' && kn.includes('페이지 열: NCS 는 줄→쪽 대응의 실제 PDF 쪽') && kt.includes('교과서는 마크다운 쪽 표식'));
...
check('S8c CLAUDE.md 의 실제 쪽 기준 수치 == summary·영향표 (공유 쪽 N, 이동 N of N, 승계 보존율, 등급3 비율 이동, NCS 쪽수 합)', claude.includes(fmt(S.meta.run.reseg_agreement.pages) + ' shared real pages all graded identically') && claude.includes(fmt(I.transition.moved) + ' of ' + fmt(N.total) + ' occurrences moved, 등급3 ' + I.grade3_share.NCS.block + ' → ' + I.grade3_share.NCS.real + '%') && claude.includes('kept only ' + I.by_source.existing.unchanged_pct + '%') && claude.includes('(' + fmt(N.groups.reduce((a, g) => a + g.pages, 0)) + ' with the two marker-only books'));

...
node   outputs/test-dashboard-data.js       # 64 assertions — semantic_summary.json ↔ data.js ↔ dashboards / analysis pages / README / CLAUDE.md, previous-basis lineage (reseg)
...
`test_semantic_keyword_recount.py` (unittest, needs a Python with `openpyxl` — `python3.13 -m unittest test_semantic_keyword_recount`) covers the semantic recount itself: rule engine, grade inheritance, corpus rules, `EXPECTED` guard, manifest, determinism, the summary/analysis-page writers. CI runs it as the last step, after the `openpyxl` install that the recount_grades absence check already needed.
...
- New logic in `semantic_keyword_recount.py` / `shift_page_markers.py` → extend `test_semantic_keyword_recount.py` (`RemediationTests` for corpus rules, guard, manifest, determinism, writers) or `test-recount-grades.py` `R8k`-`R8u` (marker shift: +1, refusals, dry-run, backup, inline marker, CRLF, `.bak` guard, CLI exit codes).

...
- **숫자 감사의 구조적 한계 (ship 적대적 리뷰 F5, 2026-09-14).** 허용 집합은 정본 값 전체의 합집합(수백 개의 작은 건수·쪽 번호·연도)이라, 다른 정본 값과 우연히 같은 1~3자리 구 수치는 감사를 통과한다 — 예: 토큰 `30` 은 키워드 수이면서 개발 분야 권수·유해인자 등급 2 건수이기도 하다(대조 JSON `keys` 가 그 다의성을 그대로 보여준다). 문단 45개가 전부 템플릿에서 나오므로 실제 위험은 낮지만, 감사는 "정본 밖 숫자가 없다" 를 보장할 뿐 "각 숫자가 맞는 자리에 있다" 를 보장하지는 않는다. 후자는 `paragraphs[].keys`·`conditions` 를 읽는 사람의 몫이다. 감사 범위는 1~3절의 모든 문단과 그 안에 중첩된 문단(표 셀·필드·메모 subList)이다.
- **산출물에 다시 실행할 수 없다 (F10).** 문단은 원본(2026-09-11) 첫머리로 찾는데 다시 쓴 문단 여럿은 첫머리가 바뀐다("둘째, 등급2는" → "둘째, 등급 2는" 등). `…_정본.hwpx` 에 다시 돌리면 "문단이 0개" 로 멈추므로, 수치가 바뀌면 원본에서 다시 만들고 산출물에 손으로 넣은 편집은 다시 넣어야 한다.
- **서술 조건의 데이터 분기 (F1·F2).** "전체 평균 수준 이상"(작업환경·중독, `_share_rel` — 정본에서는 21.6%·24.1% vs 21.9% 라 "비슷한 수준"), "등급 2 의 비중이 가장 높다"(`grade2_rank`), "‘안전’이 가장 많다"(`safety_rank`), 장비 최다 교재의 『』 호칭(`equipment_top_book`) 은 전부 조건으로 갈라지고 대조 JSON `conditions` 에 기록된다. 출처 문구(정본 날짜·사전 버전)는 `meta.run` 에서만 온다(F3).

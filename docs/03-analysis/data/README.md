...
| 파일 | 쓴 스크립트 | 뜻 | 발표 여부 |
|---|---|---|---|
| `semantic_summary.json` | `semantic_keyword_recount.py` (`EXPECTED` 가드, 2026-09-15 정본 실행) | 30개 키워드 의미 단위 재검산 + 출현별 등급 — NCS 86권 11,517건(등급 1/2/3 = 3,788/5,227/2,502; 실제 PDF 쪽 기준 `meta.page_basis.NCS = real`, 줄→쪽 대응 84권 `meta.run.page_maps`, 이전 기준과 공유 쪽 등급 일치 `meta.run.reseg_agreement` 2,035/2,035), 교과서 9권 1,207건(633/459/115) — 사전 v2(2026-09-14 결정 3), 키워드·그룹별(NCS 분야 쪽수는 PDF 쪽수 9,100), `meta.run`(git commit·명령·입력 sha256), `meta.previous_basis`(reseg 복사). `docs/semantic_recount_data.js` 와 같은 JSON | **발표 정본** (대시보드 KPI·분리 분석 페이지·README, 2026-09-13 부터; 분모 = 출현건수, 연구책임자 결정) |
...
| `hwpx_results_refresh_20260915.json` | `hwpx_results_refresh.py` | 기초보고서 HWPX 제3장 1~3절 재작성의 변경 대조(실제 쪽 기준 정본, `source.page_basis`) — 문단 45(구/신 숫자 목록·정본 키 경로 `keys`·서술 조건 결과 `conditions`)·표 7(바뀐 셀 수)·그림 3(형식·크기·비트·sha256)·숫자 감사(토큰 724, 미일치 0, 2장 5절 범위 밖 stale 0). 새 HWPX 는 `data/반도체 기초보고서_20260915_정본.hwpx`(비추적). 이름의 날짜는 정본 실행일 | 연구용 — 보고서 문장은 담지 않음 |
| `hwpx_results_refresh_20260914.json` | `hwpx_results_refresh.py` (2026-09-14, 블록 기준 정본) | 같은 재작성의 이전 실행 — 계보(토큰 723) | 계보 |
| `occurrence_real_pages_impact.json` | `occurrence_real_pages_impact.py` | 블록 기준(2026-09-14 정본 v2) vs 실제 쪽 기준(2026-09-15) — 총계 불변, NCS 등급 4,378/4,614/2,525 → 3,788/5,227/2,502, 이동 행렬 4,016건, 라벨 상속분 불변율 52.8%, 교재 종류별·키워드별·분야별, 검출 쪽 1,785 → 2,492, 출현이 놓인 블록의 실제 쪽 폭 | 연구용 — 계보(블록 기준 A 가 옛 정본과 같아야 쓴다) |

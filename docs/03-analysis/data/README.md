...
| 파일 | 쓴 스크립트 | 뜻 | 발표 여부 |
|---|---|---|---|
| `semantic_summary.json` | `semantic_keyword_recount.py` (`EXPECTED` 가드, 2026-09-14 정본 실행) | 30개 키워드 의미 단위 재검산 + 출현별 등급 — NCS 86권 12,506건(등급 1/2/3 = 5,057/4,854/2,595), 교과서 9권 1,293건(705/468/120), 키워드·그룹별, `meta.run`(git commit·명령·입력 sha256), `meta.previous_basis`(reseg 복사). `docs/semantic_recount_data.js` 와 같은 JSON | **발표 정본** (대시보드 KPI·분리 분석 페이지·README, 2026-09-13 부터; 분모 = 출현건수, 연구책임자 결정) |

...
| FR-08 대시보드 브리지·정적값 | §3.8 `page_basis real` 이면 "같은 실제 PDF 쪽 … 분모뿐" 문구, `index.html` 정적 `.ctn` 갱신 | `docs/semantic_grade_dashboard.js` 38-39 (`realNote`: 같은 실제 PDF 쪽 + 공유 쪽 N개 등급 일치 N개 + "차이는 분모(출현건수 vs 쪽)뿐"), 41 삽입; `docs/index.html` 122 (3,788 / 5,227 / 2,502, "실제 PDF 쪽에 놓고 … 공유 쪽 등급 일치 100%") | S `S1e`(브리지 문구·결속 수치) · `S5a`(정적값 == summary) · `S7a`·`S8a`(README·CLAUDE.md 비율) | 일치 (신설 `S1e` 가 기존 `S1e` 와 ID 중복 — G-6) |

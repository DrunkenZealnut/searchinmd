...
- 표 13: 헤더(교재 이름·분야·주요 내용·페이지·판정) — 마지막 열 제목을 "문장 수(글자 수)" → "판정" 으로 바꾸고, 데이터 행 13개(`accident_case_pages.json` 순서: 실제 사고 → 산업재해 아님 → 오탐), 빈 마지막 행 제거. 행 증감: 데이터 행 `hp:tr` 을 `deepcopy` 해 추가·삭제하고 `cellAddr.rowAddr` 와 `hp:tbl@rowCnt` 를 다시 매긴다. 요약 문장은 표 아래 문단(1286 뒤)이 아니라 1274 에 둔다.

...
분석 수치는 페이지 수가 아니라 문맥이 확인된 출현건수를 기준으로 계산한다. 정본(2026-09-14, 사전 v2) 전체 분모는 NCS 11,517건과 교과서 1,207건을 합한 12,724건(키워드-표현 매칭 레코드 합계 — 고유 문장·쪽 수가 아니다)이다. 페이지 수는 원문 위치를 확인하는 참고값으로만 사용한다.

| 구분 | 출현건수 (정본) | 전체 대비 |
|---|---:|---:|
| 기존·유효 정확 표현 | 9,644 | 75.8% |
| 동등 표현 추가 | 1,917 | 15.1% |
| 구체 표현 추가 | 1,163 | 9.1% |
| 문맥 제외 | 1,065 | 참고값 |

출현건수 상위 키워드는 안전 4,292건(33.7%), 위험 1,373건(10.8%), 화학물질 1,141건(9.0%), 보호구 1,068건(8.4%)이다(사전 v2: `케미컬` 보류로 화학물질 −500, 오염관리 장비어 보류·조건부로 보호구 −244). (2026-09-09 판: 14,168건 기준 안전 4,639건 32.7%, 화학물질 1,604건, 위험 1,494건, 보호구 1,360건 — 폐기.) 이 수치는 키워드별 매칭 건수의 합이므로 같은 페이지에 여러 키워드가 있으면 각각 계산된다.
